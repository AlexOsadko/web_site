...
// Показується через 90 с перебування на сайті; після закриття — повторно кожні 240 с.
...
(function () {
  var FIRST = 90000;   // перша поява — 90 секунд
  var REPEAT = 240000; // повтор після закриття — 240 секунд (4 хв)
  var VER = "19";      // зміна версії скидає збережений стан таймера (нові тайминги діють одразу)
