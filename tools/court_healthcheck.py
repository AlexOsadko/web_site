#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Перевірка «свіжості» судів у реєстрі.
...
«code|status|count|name» (без ПІБ — лише публічні назви судів і лічильники),
...
def main():
    ...
    print(f"Судів у реєстрі: {len(courts)}")
    rows = []
    ...
    for i, court in enumerate(courts, 1):
        ...
        if i % 50 == 0:
            print(f"… перевірено {i}/{len(courts)} "
    ...
    with open(f"{OUTDIR}/report.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(rows) + "\n")
    print(f"\nЗВЕДЕННЯ: ok {n_ok} · empty {n_empty} · fail {n_fail} "
          f"· усього {len(courts)}")
