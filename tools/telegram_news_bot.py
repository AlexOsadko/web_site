...
def is_ukrainian_source(text):
    """Строгіший фільтр для ОРИГІНАЛУ новини (щоб публікувати виключно українські,
    а не перекладати з російської). Українською вважаємо, лише якщо є українські
    літери (і/ї/є/ґ) і немає суто російських (ы/ъ/э/ё)."""
    t = (text or "").lower()
    ru = sum(t.count(c) for c in "ыъэё")
    ua = sum(t.count(c) for c in "іїєґ")
    return ua > 0 and ru == 0
...
SUMMARY_SYS = (
    ...
    "• ТОН — спокійний і врівноважений. Без залякування, драматизму й "
    "сенсаційності; без розмовних ярликів («витівка», «фігурант» тощо).\n"
    "ОБМЕЖЕННЯ (важливо):\n"
    ...
    "• НЕ називай конкретних покарань чи строків («аж до позбавлення волі», "
    "«до N років», «штраф N грн»), якщо їх прямо немає в джерелі. Про можливі "
    "наслідки — обережно й узагальнено («може мати серйозні правові наслідки»). "
    "Не роби юридичних висновків у конкретній справі.\n"
    ...
    "ситуацію читача.\n"
    "МОВА: пиши ВИКЛЮЧНО грамотною українською з правильною українською "
    "юридичною термінологією (напр. «ВПО», а не російське «ВПЛ»; «отримати», "
    "а не «получити»). Джерела — лише українські; нічого не перекладай.\n"
    "Поверни ЛИШЕ JSON без коментарів і markdown:\n"
    '{"headline": "короткий заголовок УКРАЇНСЬКОЮ (до ~110 символів)", '
    '"about": "2–4 речення: суть простими словами — що сталося і головне", '
    '"impact": "1–2 речення: кого це стосується і на що практично звернути увагу '
    '— спокійно, без драматизації й без переліку конкретних покарань", '
...
CASE_SYS = (
    ...
    "Виключно грамотною українською, з правильною українською термінологією "
    "(«ВПО», а не «ВПЛ»); нічого не перекладай — джерела лише українські.\n"
...
def main():
    ...
    for source, url, scope in feeds:
        try:
            ...
            for e in fp.entries[:15]:
                ...
                # Публікуємо ВИКЛЮЧНО українські новини: якщо оригінал не
                # українською (напр. російськомовні матеріали) — пропускаємо,
                # НЕ перекладаємо.
                if not is_ukrainian_source(title + " " + desc):
                    skipped_nonua += 1
                    continue
    ...
    if skipped_nonua:
        print(f"Пропущено неукраїномовних новин (публікуємо лише українські): {skipped_nonua}.")
