...
7–10 змістових розділів зі списками (1500–2500 слів), 6–8 питань у FAQ, українська мова,
...
Вимоги до вмісту:
- ОБСЯГ: розгорнута, вичерпна стаття на 1500–2500 слів. Пиши повно, не скорочуй.
- blocks: 1 вступний абзац без заголовка, далі 7–10 розділів. Кожен розділ — блок \
"h2" і після нього 2–3 блоки "p" та/або списки. За потреби всередині розділу \
використовуй підзаголовки "h3". У статті має бути щонайменше два списки.
- Обов'язково розкрий: суть питання, покрокові дії, потрібні документи чи строки, \
типові помилки, і коли варто звернутися до адвоката (окремими розділами, де доречно).
...
def validate(data, category):
    ...
    faq = data["faq"]
    if len(faq) < 5:
        raise ValueError("Потрібно щонайменше 5 питань у FAQ")
