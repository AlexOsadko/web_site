...
11 розділів H2 з повнофразовими списками (≥1000 слів), 8 питань у FAQ, українська мова,
...
- ОБСЯГ: щонайменше 1000 щільних слів (ціль 1000–2000), БЕЗ «води». Пиши повно.
- СТРУКТУРА (надійна формула обсягу): 2 вступні абзаци без заголовка, далі \
РІВНО 11 розділів H2 (10 змістовних + фінальний «З чого почати»). Кожен \
змістовний розділ — блок "h2", лід-абзац "p" на 2 речення і список "ul" на \
6–7 пунктів. КОЖЕН пункт списку — ПОВНОФРАЗОВИЙ (виділення <b>…</b> на початку \
і завершена думка у ≥10 слів), а не 3–5 слів. Фінальний розділ «З чого почати» — \
абзац "p" на ≈120–140 слів.
...
def validate(data, category):
    ...
    # Стандарт docs/article-standard.md: 8–11 H2 (ціль 11) і ≥1000 щільних слів.
    h2 = sum(1 for b in blocks if b.get("type") == "h2")
    if h2 < 8:
        raise ValueError(f"Замало розділів H2: {h2} (потрібно 8–11, ціль 11)")
    words = sum(
    ...
    if words < 1000:
        raise ValueError(f"Замало слів: {words} (потрібно ≥1000; додай розділи/пункти)")
    faq = data["faq"]
    if len(faq) < 6:
