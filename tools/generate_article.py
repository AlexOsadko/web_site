...
8–11 розділів H2 з повнофразовими списками (щільно, ~700–1500 слів), 8 питань у FAQ, українська мова,
...

{
  ...
  "blocks": [
    ...
    {"type": "p", "text": "<абзац>"},
    {"type": "h3", "text": "<підзаголовок підрозділу (за потреби)>"},
    {"type": "ul", "items": ["<пункт>", "<пункт>", "<пункт>"]},
    ...
  ],
  "faq": [
    {"q": "<коротке питання, яке гуглять>", "a": "<ґрунтовна відповідь, 2–4 речення>"},
    {"q": "...", "a": "..."},
    ...
  ]
}

Вимоги до вмісту (стандарт docs/article-standard.md):
- ГОЛОВНЕ: ЩІЛЬНІСТЬ важливіша за обсяг. Реальні норми в кожному розділі, \
БЕЗ «води». НЕ нарощуй обсяг заради числа й НЕ додавай загальних фраз-порад.
- ОБСЯГ: орієнтир ~700–1500 щільних слів (мінімум 600). Якщо тема вичерпана — \
не розбавляй.
- СТРУКТУРА: 1–2 вступні абзаци без заголовка, далі 8–11 розділів H2 (останній — \
«З чого почати»). Кожен змістовний розділ — блок "h2", лід-абзац "p" і список \
"ul" на 6–7 пунктів. КОЖЕН пункт — ПОВНОФРАЗОВИЙ і КОНКРЕТНИЙ (виділення <b>…</b> \
на початку + реальна норма/строк/сума), а не 3–5 слів. «З чого почати» — абзац \
"p" на ≈90–130 слів з практичними кроками саме для цієї теми.
- ОБОВ'ЯЗКОВІ розділи: право/підстава з нормою, «Необхідні документи» або \
«Порядок дій», строки, розмір/суми (де є), оскарження/спори, «Типові помилки», \
«З чого почати». Реальні статті кодексів і профільних законів.
- УНІКАЛЬНІСТЬ: жодних шаблонних речень-порад, що повторюються між статтями. \
Кожне речення — про конкретну тему цієї статті.
...
- faq: 6–8 пар питання–відповідь. Питання — те, що реально шукають у пошуку; \
відповіді ґрунтовні (2–4 речення), а не односкладні.
...
def validate(data, category):
    ...
    # Стандарт docs/article-standard.md: щільність важливіша за довжину.
    # 6–11 H2, ≥600 щільних слів (орієнтир 700–1500); НЕ доводити до числа «водою».
    h2 = sum(1 for b in blocks if b.get("type") == "h2")
    if not 6 <= h2 <= 11:
        raise ValueError(f"Розділів H2: {h2} (потрібно 6–11, орієнтир 8–11)")
    words = sum(
        len(str(b.get("text", "")).split())
        + sum(len(str(x).split()) for x in b.get("items", []))
        for b in blocks
    )
    if words < 600:
        raise ValueError(f"Замало слів: {words} (потрібно ≥600 щільних)")
    faq = data["faq"]
    if len(faq) < 6:
        raise ValueError("Потрібно щонайменше 6 питань у FAQ (ціль 8)")
    data["faq"] = [{"q": x["q"], "a": x["a"]} for x in faq[:8]]
    return data
